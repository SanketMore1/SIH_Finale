...
def test():
    try:
        ...
        if uploaded_file is not None:    
            ...
            st.markdown(f"<h5 style='text-align: center; color:grey'>Total number of students that are predicted to get placements are-:{len(c_df)} out of {len(c)}</h5>", unsafe_allow_html=True)
            st.header("Press on the button to generate predictions.")
            st.markdown("<h5 style='text-align: center; color:grey'>Note That The Predictions will be generated only of the features you have been selected for model training</h5>", unsafe_allow_html=True)
            for i in sections:
                if st.checkbox(i):
                    st.markdown(f"<h1 style='text-align: center;'>Predictions based on  {i}</h1>", unsafe_allow_html=True)
                    temp_df=df_cpy[i]
                    ...
                    why_df = pd.DataFrame()
                    why_df["Placed"]=final_df['Placed'].apply(lambda x: round((x/final_df["Placed"].sum())*100,2))
                    if i not in ["Gender","Region"]:
                        ax=why_df.plot.bar(y='Placed',figsize=(8, 8),xlabel=f"{i}", ylabel="Percentages of Placements",logy=True)
                    else:
                        ax=why_df.plot.bar(y='Placed',figsize=(8, 8),xlabel=f"{i}", ylabel="Percentages of Placements")
                    for p in ax.patches:
                        width = p.get_width()
                        height = p.get_height()
                        x, y = p.get_xy() 
                    ...
                    plt.title("Prediction of Placements")
                 
                    st.pyplot(plt.show())
                    final_df.plot(kind='pie', y='Placed',autopct='%1.0f%%',figsize=(8,8))
                    st.pyplot(plt.show())
        else:
